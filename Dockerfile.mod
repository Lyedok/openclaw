...
# Install extra runtime packages conditionally in a single apt layer.
# This keeps cache behavior predictable and ensures apt metadata is cleaned
# in the same layer where it is created.
ARG OPENCLAW_DOCKER_APT_PACKAGES="jq ripgrep python3 sqlite3 cmake"
...
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    ...
    set -eux; \
    base_packages="procps hostname curl git openssl ca-certificates"; \
    extra_packages="${OPENCLAW_DOCKER_APT_PACKAGES}"; \
    browser_packages=""; \
    docker_packages=""; \
    \
    if [ -n "${OPENCLAW_INSTALL_BROWSER}" ]; then \
      browser_packages="xvfb"; \
    fi; \
    \
    apt-get update; \
    ...
    if [ -n "${OPENCLAW_INSTALL_DOCKER_CLI}" ]; then \
      ...
      apt-get update; \
      docker_packages="docker-ce-cli docker-compose-plugin"; \
    fi; \
    \
...
# Bun in runtime is needed because your old image installed
# @tobilu/qmd and agent-browser globally there.
